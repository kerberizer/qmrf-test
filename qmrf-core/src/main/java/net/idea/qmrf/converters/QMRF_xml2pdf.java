...
public class QMRF_xml2pdf extends QMRFConverter {
	...
	public QMRF_xml2pdf(String ttffont) {
		...
		try {
			...
			bi_font = FontFactory.getFont(FontFactory.TIMES,
					FontFactory.defaultEncoding, true, 10, Font.BOLDITALIC); // new
																				// Font(baseFont,
																				// header_font_size,
																				// Font.BOLDITALIC);
			i_font = FontFactory.getFont(FontFactory.TIMES,
					FontFactory.defaultEncoding, true, 11, Font.BOLD); // new
																		// Font(baseFont,
																		// header_font_size,
																		// Font.ITALIC);
		} catch (Exception x) {
	...
	public void xml2pdf(InputSource xml, OutputStream pdf) {
		...
		try {
			...
			// new PdfOutline(root, new PDFAction("http://nina.acad.bg/qmrf"),
			// "a bookmark");
			...
			try {
				document.addAuthor(listNodeAttributes(doc, "qmrf_authors",
			...
			for (int i = 0; i < subchapters.length; i++)
				try {
					...
					if (Mode.chapter == (Mode) subchapters[i][2]) {
						...
						b.append(findAttributeValue(
								subchapters[i][0].toString(),
								xml_attribute_chapter, doc));
						b.append('.');
						b.append(findAttributeValue(
								subchapters[i][0].toString(),
								xml_attribute_name, doc));
						...
						PdfDestination destination = new PdfDestination(
								PdfDestination.FITH);
						PdfOutline outline = new PdfOutline(root, destination,
								bookmark);
						...
						case text: {
							createNodePhrase(subchapters[i][0].toString(), doc,
									phrase, font);
							align = Paragraph.ALIGN_JUSTIFIED;
							break;
						}
						case answer: {
							String a = findAnswer(subchapters[i][0].toString(),
									doc);
							if (a != null) {
						...
						case dataset: {
							StringBuffer b = new StringBuffer();
							b.append(findDataAvailable(
									subchapters[i][0].toString(), doc));
						...
						case attachments: {
							PdfPTable table = getAttachmentsAsTable(doc,
							...
							table = getAttachmentsAsTable(doc,
							 ...
							 * b.append(listAttachments
							 * (doc,"attachment_training_data"));
							 * b.append("Test set(s)\n");
							 * b.append(listAttachments
							 * (doc,"attachment_validation_data"));
							 * b.append("Supporting information\n");
							 * b.append(listAttachments
							 * (doc,"attachment_documents")); Chunk attachments
							 * = new Chunk(b.toString());
						...
						case reference: {
							try {
								String value = listNodeAttributes(doc,
										subchapters[i][0].toString(),
						...
						float pos = writer.getVerticalPosition(false);
						/*
						 * cb.moveTo(0, pos); cb.lineTo(PageSize.A4.width(),
						 * pos); cb.stroke();
						 */
						if (pos < 90)
							document.newPage();
	...
	protected PdfPTable getAttachmentsAsTable(org.w3c.dom.Document doc,
			String attachments) {
					...
					if (attachment.getLength() == 0)
						...
						for (int j = 0; j < attachment.getLength(); j++)
							if (attachment.item(j) instanceof org.w3c.dom.Element) {
								org.w3c.dom.Element e = ((org.w3c.dom.Element) attachment
										.item(j));
	...
	public void headerTable(Document pdfdoc, org.w3c.dom.Document xmldoc) {
		try {
			...
			try {
				...
				Chunk ident_title = new Chunk(
						"QMRF identifier (JRC Inventory):", bi_font);
	...
	/*
	 * protected String findNodeValue(String name_node, org.w3c.dom.Document
	 * source) {
	 * 
	 * NodeList n = source.getElementsByTagName(name_node); if (n.getLength()
	 * ==0) return ""; String v = n.item(0).getTextContent(); // return
	 * org.apache.commons.lang.StringEscapeUtils.unescapeXml(replaceTags(v));
	 * try { System.out.println(v); org.w3c.dom.Document doc =
	 * nodeBuilder.parse(new InputSource(new StringReader(v.trim())));
	 * StringBuffer b = new StringBuffer(); org.w3c.dom.Element e =
	 * doc.getDocumentElement(); getText(e, b,font); return b.toString(); }
	 * catch (Exception x) { // x.printStackTrace(); return
	 * org.apache.commons.lang.StringEscapeUtils.unescapeXml(replaceTags(v)); }
	 * }
	 */
	protected void createNodePhrase(String name_node,
			org.w3c.dom.Document source, Phrase phrase, Font font) {
		...
		// org.apache.commons.lang.StringEscapeUtils.unescapeXml(replaceTags(v));
		try {
			org.w3c.dom.Document doc = nodeBuilder.parse(new InputSource(
					new StringReader(v)));
			...
			Chunk chunk = new Chunk(
					org.apache.commons.lang.StringEscapeUtils
							.unescapeXml(replaceTags(v)));
			chunk.setFont(font);
			phrase.add(chunk);
		}
	}

	protected int getText(org.w3c.dom.Node node, Phrase phrase,
			Font currentFont, ScriptMode scriptMode, boolean trim,
			int paragraphs) {
		...
		if (node.getNodeType() == node.ELEMENT_NODE) {
			...
			if ("b".equals(node.getNodeName())) {
				if ((currentFont.style() == Font.ITALIC)
						|| (currentFont.style() == Font.BOLDITALIC))
			...
			if ("i".equals(node.getNodeName())) {
				if ((currentFont.style() == Font.BOLD)
						|| (currentFont.style() == Font.BOLDITALIC))
					fweight = Font.BOLDITALIC;
				else
					fweight = Font.ITALIC;
			...
			if (modify) {
				f = FontFactory.getFont(currentFont.getFamilyname(), fsize,
						fweight, clr);
			}
			...
			for (int i = 0; i < nodes.getLength(); i++)
				paragraphs += getText(nodes.item(i), phrase, f, scriptMode,
						trim, paragraphs);
			...
			if (trim)
				value = replaceBlanks(value).trim();
			if ("".equals(value))
				return paragraphs;
