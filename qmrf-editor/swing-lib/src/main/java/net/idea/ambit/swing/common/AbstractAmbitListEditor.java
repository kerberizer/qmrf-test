...
public class AbstractAmbitListEditor<L extends AmbitList> extends JPanel implements IAmbitEditor<L> {
		...
		for (int i = list.size() - 1; i >= 0; i--) {
			if (list.getItem(i).isSelected())
				if (JOptionPane.showConfirmDialog(this,
						"Delete " + list.getItem(i).toString() + "?") == JOptionPane.OK_OPTION) {
	...
	public JToolBar createJToolbar() {
		...
		newAction = new AbstractActionWithTooltip("", UITools.createImageIcon("ambit/ui/images/table_row_insert.png"),
				"Add new item") {
		...
		deleteAction = new AbstractActionWithTooltip("",
				UITools.createImageIcon("ambit/ui/images/table_row_delete.png"), "Delete selected items") {
			public void actionPerformed(ActionEvent e) {
				try {
					deleteSelected();
				} catch (Exception x) {
		...
		/*
		 * editBox = new
		 * JToggleButton(UITools.createImageIcon("ambit/ui/images/draw_16.png"),
		 * false); editBox.setFocusable(false);
		 * editBox.setEnabled(list.isEditable());
		 * editBox.setToolTipText("Enable/disable editing of the current item");
		 * editBox.addItemListener(new ItemListener() { public void
		 * itemStateChanged(ItemEvent e) { Object source =
		 * e.getItemSelectable(); if (e.getStateChange() ==
		 * ItemEvent.DESELECTED) editItem(false); else if (e.getStateChange() ==
		 * ItemEvent.SELECTED) editItem(true);
		 * 
		 * } }); editBox.setMinimumSize(new Dimension(24,24));
		 * editBox.setPreferredSize(new Dimension(24,24)); toolbar.add(new
		 * Separator()); toolbar.add(editBox);
		 */
