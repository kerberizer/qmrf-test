...
public abstract class QMRFConverter {
	...
	final String[] att_author = { "name", "affiliation", "contact", "email",
			"url" };
	...
	/**
	 * HtmlCleaner claims thread safety 
	 * http://htmlcleaner.sourceforge.net/javause.php
	 */
	protected static transient HtmlCleaner cleaner = new HtmlCleaner();
	...
	public Object[][] subchapters = {
			...
			{ "QSAR_software", "QSAR_software_text", Mode.reference,
					"software_ref", "software", att_software,
					new Boolean(false) },

			{ "QSAR_General_information", "QSAR_General_information",
					Mode.chapter },
			{ "qmrf_date", "qmrf_date", Mode.title },
			{ "qmrf_date", "qmrf_date_text", Mode.text },

			{ "qmrf_authors", "qmrf_authors", Mode.title },
			{ "qmrf_authors", "qmrf_authors_ref", Mode.reference, "author_ref",
					"author", att_author, new Boolean(true) },
			...
			{ "model_authors", "model_authors_ref", Mode.reference,
					"author_ref", "author", att_author, new Boolean(true) },

			{ "model_date", "model_date", Mode.title },
			{ "model_date", "model_date_text", Mode.text },

			{ "references", "references", Mode.title },
			{ "references", "qmrf_date_text", Mode.reference,
					"publication_ref", "publication", att_reference,
					new Boolean(true) },
			...
			{ "model_endpoint", "model_endpoint_text", Mode.reference,
					"endpoint_ref", "endpoint", att_endpoint, new Boolean(true) },
			...
			{ "algorithm_explicit", "algorithm_explicit_text", Mode.reference,
					"algorithm_ref", "algorithm", att_algorithm,
					new Boolean(false) },
			...
			{ "algorithms_descriptors", "algorithms_descriptors_text",
					Mode.reference, "descriptor_ref", "descriptor",
					att_descriptor, new Boolean(true) },
			...
			{ "descriptors_generation", "descriptors_generation_text",
					Mode.text },

			{ "descriptors_generation_software",
					"descriptors_generation_software", Mode.title },
			{ "descriptors_generation_software",
					"descriptors_generation_software_text", Mode.reference,
					"software_ref", "software", att_software,
					new Boolean(false) },

			{ "descriptors_chemicals_ratio", "descriptors_chemicals_ratio",
					Mode.title },
			{ "descriptors_chemicals_ratio",
					"descriptors_chemicals_ratio_text", Mode.text },

			{ "QSAR_Applicability_domain", "QSAR_Applicability_domain",
					Mode.chapter },

			{ "app_domain_description", "app_domain_description", Mode.title },
			{ "app_domain_description", "app_domain_description_text",
					Mode.text },
			...
			{ "app_domain_software", "app_domain_software_text",
					Mode.reference, "software_ref", "software", att_software,
					new Boolean(false) },
			...
			{ "training_set_availability", "training_set_availability",
					Mode.title },
			{ "training_set_availability", "training_set_availability_text",
					Mode.answer },
			...
			{ "training_set_descriptors", "training_set_descriptors",
					Mode.title },
			{ "training_set_descriptors", "training_set_descriptors_text",
					Mode.answer },

			{ "dependent_var_availability", "dependent_var_availability",
					Mode.title },
			{ "dependent_var_availability", "dependent_var_availability_text",
					Mode.answer },
			...
			{ "validation_set_availability", "validation_set_availability",
					Mode.title },
			{ "validation_set_availability",
					"validation_set_availability_text", Mode.answer },

			{ "validation_set_data", "validation_set_data", Mode.title },
			{ "validation_set_data", "validation_set_data_text", Mode.text },
			{ "validation_set_data", "validation_set_data_dataset",
					Mode.dataset },

			{ "validation_set_descriptors", "validation_set_descriptors",
					Mode.title },
			{ "validation_set_descriptors", "validation_set_descriptors_text",
					Mode.answer },

			{ "validation_dependent_var_availability",
					"validation_dependent_var_availability", Mode.title },
			{ "validation_dependent_var_availability",
					"validation_dependent_var_availability_text", Mode.answer },
			...
			{ "validation_predictivity", "validation_predictivity_text",
					Mode.text },
			...
			{ "mechanistic_basis_comments", "mechanistic_basis_comments",
					Mode.title },
			{ "mechanistic_basis_comments", "mechanistic_basis_comments_text",
					Mode.text },

			{ "mechanistic_basis_info", "mechanistic_basis_info", Mode.title },
			{ "mechanistic_basis_info", "mechanistic_basis_info_text",
					Mode.text },
			...
			{ "bibliography", "bibliography_text", Mode.reference,
					"publication_ref", "publication", att_reference,
					new Boolean(true) },
	...
	protected static Pattern HTML_tags = Pattern
			.compile("(<html>|</html>|<head>|</head>|<title>|</title>|<body>|</body>|<b>|</b>|<i>|</i>|\t)");
	protected static Pattern ptag = Pattern
			.compile("(<p>|<p style=\"margin-top: 0\">|</p>)");
	...
	public static String replaceTags(String text) {
		return replaceTagsHTMLCleaner(text);
	}

	public static String replaceTagsHTMLCleaner(String text) {
		...
		node.traverse(new TagNodeVisitor() {
		    ...
		    public boolean visit(TagNode tagNode, HtmlNode htmlNode) {
	            ...
	            if (htmlNode instanceof ContentNode) {
	            	builder.append(replaceBlanks(replaceNewLine(
	            			((ContentNode)htmlNode).getContent().toString())
	            			));
	            } else if (htmlNode instanceof TagNode) {
		            TagNode tag = (TagNode) htmlNode;
		            try {
		            	supported_tag tagName = supported_tag.valueOf(tag.getName());
		            	switch (tagName) {
		            	case style: {  tag.removeFromTree(); break;}
		            	case head: {  tag.removeFromTree(); break;}
		            	case font: break;
		            	case img: break;
		            	case a: break;
		            	case p: {builder.append("\n"); break;}
		            	default : tag.getAttributes().clear();
		            	}
		            } catch (Exception x) {
	...
	/**
	 * Provided that text is html formatted, leaves only selected tags.
	 * The primary reason is to leave bare bone html, minus MSWord garbage.
	 * @param text
	 * @return
	 */
	public static String htmlBoning(InputStream text) throws IOException {
		return htmlBoning(new InputStreamReader(text));
	}
	public static String htmlBoning(Reader text) throws IOException {
		    ...
		    public boolean visit(TagNode tagNode, HtmlNode htmlNode) {
	            ...
	            if (htmlNode instanceof ContentNode) {
		            ...
		            try {
		            	...
		            	case style: {  tag.removeFromTree(); break;}
		            	case head: {  tag.removeFromTree(); break;}
		            	//case font: {  tag.removeAttribute("face"); tag.removeAttribute("size"); break;} 
		            	case img:  {  tag.removeFromTree(); break;}
		            	case form: {  tag.removeFromTree(); break;}
		            	case script: {  tag.removeFromTree(); break;}
		            	case input: {  tag.removeFromTree(); break;}
		            	case a: break;
		            	default : {
		            		Iterator<String> i = tag.getAttributes().keySet().iterator();
		            		while (i.hasNext()) {
		            			tag.removeAttribute(i.next());
		            		}
		            	}
		            	}
		            } catch (Exception x) {
		...
		CompactHtmlSerializer serializer =  new CompactHtmlSerializer(cleaner.getProperties());
		try {
			ByteArrayOutputStream baos = new ByteArrayOutputStream();
			serializer.writeToStream(node, baos);
	...
	public static String replaceTagsRegexp(String text) {
		if (text == null)
			return text;
	...
	/*
	 * !ATTLIST attachments chapter CDATA #FIXED "9.3" name CDATA #FIXED
	 * "Supporting information" help CDATA ""> <!ELEMENT
	 * attachment_training_data (molecules*) > <!ELEMENT
	 * attachment_validation_data (molecules*) > <!ELEMENT attachment_documents
	 * (document*)>
	 */
	...
	public String listNodeAttributes(Document doc, String name_node,
			String ref_name, String catalog_name, String[] attributes,
			boolean line) throws Exception {
		ArrayList node_list = findNodeIDRef(name_node, ref_name, catalog_name,
				doc);
		...
		for (Iterator it = node_list.iterator(); it.hasNext();) {
			...
			for (int i = 0; i < attributes.length; i++)
				try {
					String value = findAttributeValue(attributes[i], node)
							.trim();
					if (value.length() > 0) {
						b.append(value);
						if (line)
	...
	protected String findAttributeValue(String name_node,
			String name_attribute, org.w3c.dom.Document source) {
	...
	protected String findAttributeValue(String name_attribute, Node source)
			throws Exception {
	...
	public static ArrayList findNodeIDRef(String name_node, String ref_name,
			String catalog_name, org.w3c.dom.Document source) throws Exception {
		...
		for (int i = 0; i < objNodes.getLength(); i = i + 1) {
			...
			if (objNodes.item(i).getNodeName().equals(ref_name)) {
				...
				NodeList objNodesAuthor = source
						.getElementsByTagName(catalog_name);
				for (int j = 0; j < objNodesAuthor.getLength(); j = j + 1) {
					String id = objNodesAuthor.item(j).getAttributes()
							.getNamedItem("id").getNodeValue();
					if (id.equals(attribute.getNodeValue())) {
						// Node name =
						// objNodesAuthor.item(j).getAttributes().getNamedItem(xml_attribute_name);
	...
	protected String findDataAvailable(String name_node,
	...
	protected String findNodeValue(String name_node, org.w3c.dom.Document source) {
		...
		return org.apache.commons.lang.StringEscapeUtils.unescapeXml(
				replaceTags(v)).trim();

	}

	public static Color Hex2Color(String clr) throws Exception {
		return new Color(Integer.valueOf(clr.substring(0, 2), 16),
				Integer.valueOf(clr.substring(2, 4), 16), Integer.valueOf(
						clr.substring(4, 6), 16));
	}
}
