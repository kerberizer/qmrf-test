...
public class AttachmentHTMLReporter extends QMRFHTMLReporter<DBAttachment, IQueryRetrieval<DBAttachment>> {
	...
	if (protocol != null) {
	    ...
	    setTitle(String.format("<a href='%s%s/%s'>%s</a> attachment", request.getRootRef(), Resources.protocol,
		    qmrf, protocol.getVisibleIdentifier()));
	...
	return clientInfo.getRoles().indexOf(managerRole) >= 0 || clientInfo.getRoles().indexOf(editorRole) >= 0;
    ...
    @Override
    protected void printPageNavigator(IQueryRetrieval<DBAttachment> query) throws Exception {
	try {
	    String url = "<li>%s</li>";
	    output.write("<div><ul id='hnavlist'>");
	    if (documentUI != null)
		output.write(String.format(url, documentUI));
	    if (allAttachmentsUI != null)
		output.write(String.format(url, allAttachmentsUI));
	...
	if (headless)
	    ...
	    datasets.append(String.format("<a href='%s/A%d'>%s</a>", "attachment", attachment.getID(),
		    attachment.getDescription()));
	datasets.append("</th>");
	datasets.append(String.format("<td align='left'>%s</td>", attachment.getType()));
	datasets.append(String.format("<td align='left'><a href='%s?media=%s' title='%s %s'>Download</a></td>", uri,
		Reference.encode(attachment.getMediaType()), attachment.getTitle(), attachment.getMediaType()));
	...
	case document: {
	    ...
	    if (isAdminOrEditor()) {
		String form = String
			.format("<form method='POST' title='Remove attachment' action='%s?method=delete'><input type='hidden' value='true' name='delete'><input type='submit' class='Draw' title='Delete attachment' value='Delete attachment'></form>",
				uri);
	...
	default: {
		...
		String datasetURI = String.format("%s%s/A%d", uriReporter.getBaseReference(), Resources.dataset,
			attachment.getID());

		datasets.append(String.format("<td align='left' ><a href='%s'>Browse structures</a></td>", datasetURI));
		datasets.append("<td></td>");
	    } else {

		if (isAdminOrEditor()) {
		    String form = String
			    .format("<form method='POST' title='This dataset is not yet browsable and searchable' action='%s/dataset'><input type='hidden' value='true' name='import'><input type='submit' class='Draw' title='Convert to browsable and searchable dataset' value='Convert to browsable'></form>",
				    uri);
		    datasets.append(String.format("<td align='left'>%s</td>", form));
		    form = String
			    .format("<form method='POST' title='Remove attachment' action='%s?method=delete'><input type='hidden' value='true' name='delete'><input type='submit' class='Draw' title='Delete attachment' value='Delete attachment'></form>",
	...
	try {
	    if (uploadUI != null && headless)
		output.write(uploadUI);
    ...
    @Override
    protected void printForm(Writer output, String uri, DBAttachment item, boolean editable) {
