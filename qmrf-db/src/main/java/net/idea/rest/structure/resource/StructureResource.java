...
public class StructureResource extends QMRFCatalogResource<Structure> {
	...
	String search = form.getFirstValue(QueryResource.search_param) == null ? "" : form.getFirstValue(
		QueryResource.search_param).trim();
	if ((search != null) && search.contains("script"))
	    search = null; // not expected here

	if ((search == null) || "".equals(search))
	    search = "benzene"; // let's have a default
	...
	try {
	    ...
	    parameters.setModels(null);
	}
    }

    protected Reference getSearchReference(Context context, Request request, Response response,
	    StructureHTMLBeauty parameters) throws ResourceException {
	...
	try {
		...
		ref = new Reference(String.format("%s/query/similarity?threshold=%3.2f", queryService,
		...
		if (parameters.getSearchQuery().indexOf("#") >= 0) // this
								   // breasks
								   // jquery
								   // jsonp even
								   // if url
	    ...
	    throw createException(Status.CLIENT_ERROR_BAD_REQUEST, parameters.getSearchQuery(), parameters.option,
		    ref.toString(), x);
	}
    }

    @Override
    protected Iterator<Structure> createQuery(Context context, Request request, Response response)
	...
	throw new ResourceException(status.getCode(), message, String.format(
		"Error when contacting (%s) structure search service at %s", option.toString(), ref),
		"http://www.w3.org/Protocols/rfc2616/rfc2616-sec10.html", x);
    }

    @Override
    public IProcessor<Iterator<Structure>, Representation> createJSONConvertor(Variant variant, String filenamePrefix)
	...
	if (htmlBeauty == null)
	    htmlBeauty = new StructureHTMLBeauty(queryService);
	return htmlBeauty;
    }
	...
	if (modelsKey == null || modelsKey.length == 0)
	    return null;
...
class PropertiesIterator extends CSVFeatureValuesIterator<Structure> {
	...
	for (int i = 0; i < header.size(); i++)
		...
		Structure._titles title = Structure._titles.valueOf(header.get(i).toString()
