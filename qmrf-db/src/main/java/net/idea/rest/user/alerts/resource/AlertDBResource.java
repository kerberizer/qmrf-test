 ...
 * @param <Q>
 */
public class AlertDBResource extends QMRFQueryResource<ReadAlert, DBAlert> {
	...
	if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
	    return new StringConvertor(new AlertURIReporter<IQueryRetrieval<DBAlert>>(getRequest()),
	    ...
	    return new RDFJenaConvertor<DBAlert, IQueryRetrieval<DBAlert>>(
		    new AlertRDFReporter<IQueryRetrieval<DBAlert>>(getRequest(), variant.getMediaType(),
	    ...
	    return new OutputWriterConvertor(createJSONReporter(), MediaType.TEXT_HTML);
	else
	    throw new ResourceException(Status.CLIENT_ERROR_UNSUPPORTED_MEDIA_TYPE);
    }
    @Override
    protected QueryReporter createJSONReporter() throws ResourceException {
	AlertJSONReporter rep = new AlertJSONReporter(getRequest());
	return rep;
    }
	...
	if (userKey != null) {
	    ...
	    throw new ResourceException(Status.CLIENT_ERROR_BAD_REQUEST, "Empty usUSer");
	return user;
    }

    protected ReadAlert getAlertQuery(Object key, Object userKey, String search_name, Object search_value)
	...
	if (key == null) {
	    ...
	    /*
	     * if (search_value != null) { if ("search".equals(search_name)) {
	     * DBUser user = new DBUser(); String s = String.format("^%s",
	     * search_value.toString()); user.setLastname(s);
	     * user.setFirstname(s); query.setValue(user); } else if
	     * ("username".equals(search_name)) { DBUser user = new DBUser();
	     * query.setCondition(EQCondition.getInstance());
	     * user.setUserName(search_value.toString()); query.setValue(user);
	     * } }
	     */
	    return query;
	} else {
    ...
    @Override
    protected ReadAlert createQuery(Context context, Request request, Response response) throws ResourceException {
    ...
    @Override
    protected QueryURIReporter<DBAlert, ReadAlert> getURUReporter(Request baseReference) throws ResourceException {
	return new AlertURIReporter(getRequest());
    }

    @Override
    protected CallableProtectedTask<String> createCallable(Method method, Form form, DBAlert item)
	    throws ResourceException {
	...
	try {
	    ...
	    return new CallableAlertCreator(method, item, user, reporter, form, getRequest().getRootRef().toString(),
		    conn, getToken());
	} catch (Exception x) {
	    try {
		conn.close();
    ...
    @Override
    protected ReadAlert createUpdateQuery(Method method, Context context, Request request, Response response)
	    throws ResourceException {
	Object key = request.getAttributes().get(AlertDBResource.resourceKey);
	Object userKey = request.getAttributes().get(UserDBResource.resourceKey);
	if (Method.POST.equals(method)) {
	    if ((userKey != null)
		    || ((getRequest().getClientInfo().getUser() != null) && (getRequest().getClientInfo().getUser()
			    .getIdentifier() != null))) {
		if (key == null)
		    return null;// post allowed only on /alert level, not on
				// /user/id
	    }
	} else {
	    if (key != null)
	...
	if (htmlBeauty == null)
	    htmlBeauty = new QMRF_HTMLBeauty(Resources.alert);
	return htmlBeauty;
    }
}
