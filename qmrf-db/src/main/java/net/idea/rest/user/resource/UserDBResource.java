...
public class UserDBResource<T> extends QMRFQueryResource<ReadUser<T>, DBUser> {
	...
	if (variant.getMediaType().equals(MediaType.TEXT_URI_LIST)) {
	    ...
	    return new OutputWriterConvertor(new net.idea.rest.user.resource.UserJSONReporter(getRequest(),
		    readRegistrationStatus ? usersdbname : null), MediaType.APPLICATION_JSON);
	} else if (variant.getMediaType().equals(MediaType.TEXT_XML)) {
	    return new OutputWriterConvertor(
		    new UserXMLReporter<IQueryRetrieval<DBUser>>(getRequest().getResourceRef()), MediaType.TEXT_XML);
	...
	UserHTMLReporter rep = new UserHTMLReporter(getRequest(), !singleItem, editable,
		(UserHTMLBeauty) getHTMLBeauty(), readRegistrationStatus ? usersdbname : null);
    ...
    @Override
    protected QueryURIReporter<DBUser, ReadUser<T>> getURUReporter(Request baseReference) throws ResourceException {
	...
	if (htmlBeauty == null)
	    htmlBeauty = new UserHTMLBeauty(Resources.user);
	return htmlBeauty;
    }
