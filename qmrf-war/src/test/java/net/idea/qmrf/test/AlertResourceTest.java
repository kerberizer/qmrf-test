...
public class AlertResourceTest extends ResourceTest {
	...
	while ((line = r.readLine()) != null) {
	    Assert.assertTrue(line.startsWith(String.format("http://localhost:%d%s/U3%s/A", port, Resources.user,
		    Resources.alert)));
	    count++;
	}
	return count == 2;
     ...
     * testGet(String.format("http://localhost:%d%s/U3%s",
     * port,Resources.user,Resources.alert), MediaType.APPLICATION_RDF_XML); }
     * 
     ...
     * 
     * AlertIO ioClass = new AlertIO(); List<Alert> alerts =
     * ioClass.fromJena(model); Assert.assertEquals(2,alerts.size());
     * Assert.assertTrue( alerts.get(0).getResourceURL().toString().startsWith(
     * String
     * .format("http://localhost:%d%s/U3%s/A",port,Resources.user,Resources
     * .alert) )); Assert.assertTrue("cell".equals(alerts.get(0).getTitle()) ||
     * "toxicity".equals(alerts.get(0).getTitle()) ) ;
     * Assert.assertTrue("cell".equals(alerts.get(0).getQueryString()) ||
     * "toxicity".equals(alerts.get(0).getQueryString()) ) ;
     * 
     * Assert.assertTrue(Alert.RecurrenceFrequency.daily.equals(alerts.get(0).
     * getRecurrenceFrequency()) ||
     * Alert.RecurrenceFrequency.weekly.equals(alerts
     * .get(0).getRecurrenceFrequency()));
     ...
     * Assert.assertNotNull(alerts.get(0).getUser()); String user =
     * String.format("http://localhost:%d%s/U3",port,Resources.user);
     * Assert.assertEquals(user,
     * alerts.get(0).getUser().getResourceURL().toString()); return model; }
     */
    @Override
	...
	RemoteTask task = testAsyncPoll(
		new Reference(String.format("http://localhost:%d%s/U3%s", port, Resources.user, Resources.alert)),
		MediaType.TEXT_URI_LIST, new UrlEncodedFormEntity(form, "UTF-8"), Method.POST);
	// wait to complete
	while (!task.isDone()) {
	    task.poll();
	    Thread.sleep(100);
	...
	Assert.assertTrue(task.getResult().toString()
		.startsWith(String.format("http://localhost:%d%s/U3%s/A", port, Resources.user, Resources.alert)));

	c = getConnection();
	table = c.createQueryTable("EXPECTED",
		"SELECT idquery,name,query,qformat,rfrequency,rinterval,iduser,created FROM alert");
	Assert.assertEquals(3, table.getRowCount());
	table = c.createQueryTable("EXPECTED", "SELECT idquery,iduser,query from alert where idquery>2");
	Assert.assertEquals(1, table.getRowCount());

	String expectedURI = String.format("http://localhost:%d%s/U3%s/A%s", port, Resources.user, Resources.alert,
		table.getValue(0, "idquery"));
