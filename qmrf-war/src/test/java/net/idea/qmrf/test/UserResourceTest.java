...
import net.idea.opentox.cli.user.User;
import net.idea.opentox.cli.user.UserClient;
...
public class UserResourceTest extends ResourceTest {
     ...
     * 
     * UserIO ioClass = new UserIO(); List<User> users =
     * ioClass.fromJena(model); Assert.assertEquals(1,users.size());
     * Assert.assertEquals
     * (String.format("http://localhost:%d%s/U3",port,Resources.user),
     * users.get(0).getResourceURL().toString()); Assert.assertEquals("",
     * users.get(0).getTitle());
     * 
     * Assert.assertEquals("http://qsardb.jrc.it",
     * users.get(0).getHomepage().toString()); Assert.assertEquals("abcdef",
     * users.get(0).getFirstname()); Assert.assertEquals("ABCDEF",
     ...
     * Assert.assertEquals(1,users.get(0).getProjects().size()); return model; }
     */

    @Test
    public void testJSON() throws Exception {
	testGet(String.format("http://localhost:%d%s/U3", port, Resources.user), MediaType.APPLICATION_JSON);
    }
	...
	try {
	    UserClient cli = new UserClient(httpclient);
	    List list = verifyResponseJSON(uri, media, in, cli);
	    Assert.assertNotNull(list);
	    Assert.assertEquals(1, list.size());
	    Assert.assertEquals(String.format("http://localhost:%d%s/U3", port, Resources.user), ((User) list.get(0))
		    .getResourceIdentifier().toExternalForm());
	    Assert.assertEquals("", ((User) list.get(0)).getTitle());
	    Assert.assertEquals("http://qsardb.jrc.it", ((User) list.get(0)).getHomepage().toString());
	    Assert.assertEquals("abcdef", ((User) list.get(0)).getFirstName());
	    Assert.assertEquals("ABCDEF", ((User) list.get(0)).getLastName());
	    /*
	     * Assert.assertNotNull(((User) list.get(0)).getOrganisations());
	     * Assert.assertEquals(1,((User)
	     * list.get(0)).getOrganisations().size());
	     * Assert.assertEquals(1,((User) list.get(0)).getProjects().size());
	     */
	    return true;
	} finally {
	...
	formparams.add(new BasicNameValuePair("pwd1", "testtest123456"));
	formparams.add(new BasicNameValuePair("pwd2", "testtest123456"));
	...
	RemoteTask task = testAsyncPoll(new Reference(String.format("http://localhost:%d%s", port, Resources.user)),
	...
	Assert.assertTrue(task.getResult().toString()
		.startsWith(String.format("http://localhost:%d/register/notify", port)));
	...
	table = c.createQueryTable("EXPECTED",
		"SELECT iduser,title from user where iduser>5 and firstName='Alice' and lastName='B.'");
	...
	testCreateEntryFromWebForm(Resources.user);
	    ...
	    Assert.assertTrue(x.getMessage().indexOf("Privacy")>=0);
	}
    }
	...
	formparams.add(new BasicNameValuePair("organisation_uri", String.format("http://localhost:%d%s/G1", port,
		Resources.organisation)));
	// formparams.add(new BasicNameValuePair("organisation_uri",
	// String.format("http://localhost:%d%s/G2",port,Resources.organisation)));
	formparams.add(new BasicNameValuePair("project_uri", String.format("http://localhost:%d%s/G2", port,
		Resources.project)));
	...
	RemoteTask task = testAsyncPoll(new Reference(String.format("http://localhost:%d%s", port, resource)),
		MediaType.TEXT_URI_LIST, new UrlEncodedFormEntity(formparams, "UTF-8"), Method.POST);
	// wait to complete
	...
	Assert.assertEquals(1, table.getRowCount());
	table = c.createQueryTable("EXPECTED",
		"SELECT iduser,idorganisation from user_organisation where iduser>88 and idorganisation=1");
	Assert.assertEquals(1, table.getRowCount());
	table = c
		.createQueryTable(
			"EXPECTED",
			"SELECT iduser,idorganisation,name from user_organisation join organisation using(idorganisation) where iduser>88 and name='My new affiliation'");
	Assert.assertEquals(1, table.getRowCount());
	table = c.createQueryTable("EXPECTED",
		"SELECT iduser,idproject from user_project where iduser>88 and idproject=2");
	Assert.assertEquals(1, table.getRowCount());
	c.close();
	...
	RemoteTask task = testAsyncPoll(new Reference(String.format("http://localhost:%d%s/U3", port, Resources.user)),
		MediaType.TEXT_URI_LIST, new UrlEncodedFormEntity(formparams, "UTF-8"), Method.PUT);
	// wait to complete
	while (!task.isDone()) {
	    task.poll();
	    Thread.sleep(100);
