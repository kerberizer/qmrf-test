...
public abstract class ResourceTest extends DbUnitTest {
	...
	setUpDatabase("src/test/resources/net/idea/qmrf/tables.xml");
    }
	...
	HttpClient cli = new DefaultHttpClient();
	RemoteTask task = new RemoteTask(cli, new URL(uri), MediaType.TEXT_URI_LIST.toString(), rep,
    ...
    public List verifyResponseJSON(String uri, MediaType media, InputStream in, AbstractURIClient cli) throws Exception {
	if (cli==null) throw new Exception("No client provided");
	return cli.processPayload(in, media.getName());
    }
    ...
    protected MultipartEntity getMultipartWebFormRepresentation(String[] formFields, String[] formValues, File file,
	    String mediaType) throws Exception {
	return getMultipartWebFormRepresentation(formFields, formValues, "filename", file, mediaType);
    }

    protected MultipartEntity getMultipartWebFormRepresentation(String[] formFields, String[] formValues,
