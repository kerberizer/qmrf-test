...
public class ProjectResourceTest extends ResourceTest {
	...
	testGet(String.format("http://localhost:%d%s/G1", port, Resources.project), MediaType.APPLICATION_JSON);
    }
	...
	try {
	    ...
	    Assert.assertEquals("N/A", ((Group) list.get(0)).getTitle());
	    return true;
	} finally {
	...
	RemoteTask task = testAsyncPoll(new Reference(String.format("http://localhost:%d%s", port, Resources.project)),
	...
	List<NameValuePair> formparams = new ArrayList<NameValuePair>();
	formparams.add(new BasicNameValuePair("project_uri", String.format("http://localhost:%d%s/G1", port,
		Resources.project)));
