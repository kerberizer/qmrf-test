...
import net.idea.opentox.cli.dataset.alerts.AlertClient;
...
public class ProtocolResourceTest extends ProtectedResourceTest {
	...
	return String.format("http://localhost:%d%s/%s", port, Resources.protocol, id2v2);
    }
	...
	while ((line = r.readLine()) != null) {
	    Assert.assertEquals(String.format("http://localhost:%d%s/%s", port, Resources.unpublished, id2v2), line);
	    count++;
	}
	return count == 1;
    }
	...
	try {
	    QMRFDocumentClient cli = new QMRFDocumentClient(httpclient);
    ...
    /*
     * 
     * @Test public void testRDF() throws Exception { testGet(getTestURI(),
     * MediaType.APPLICATION_RDF_XML); }
     * 
     * @Override public OntModel verifyResponseRDFXML(String uri, MediaType
     * media, InputStream in) throws Exception {
     * 
     * OntModel model = ModelFactory.createOntologyModel(); model.read(in,
     * null);
     * 
     * ProtocolIO ioClass = new ProtocolIO(); List<Protocol> protocols =
     * ioClass.fromJena(model); Assert.assertEquals(1, protocols.size());
     * Assert.assertEquals(String.format(
     * "http://localhost:8181/protocol/%s",id2v2),
     * protocols.get(0).getResourceURL().toString()); Assert.assertEquals(id2v2,
     * protocols.get(0).getIdentifier());
     * Assert.assertEquals("QSAR model for narcosis", protocols.get(0)
     * .getTitle()); Assert.assertNotNull(protocols.get(0).getAbstract());
     * //Assert.assertEquals(5,
     * protocols.get(0).getAbstract().indexOf("\u2122")); // TM // symbol
     * 
     * Assert.assertFalse(protocols.get(0).isPublished());
     * Assert.assertNotNull(protocols.get(0).getOwner()); Assert.assertEquals(
     ...
     * protocols.get(0).getOwner().getFirstname()); return model; }
     */
    /*
	...
	c.close();
	String org = String.format("http://localhost:%d%s/%s", port, Resources.protocol, "Q8-10-13-121");
	...
	c = getConnection();
	table = c.createQueryTable("EXPECTED", "SELECT * FROM protocol where idprotocol=121 and version=1");
	...
	String uri = String.format("http://localhost:%d%s/%s", port, Resources.protocol, id2v2);
	String newURI = createEntryFromMultipartWeb(new Reference(uri), Method.PUT);
	String newQMRF = "Q15-14-0001";
	IDatabaseConnection c = getConnection();
	...
	String url = createEntryFromMultipartWeb(new Reference(String.format("http://localhost:%d%s/%s%s", port,
	...
	Assert.assertEquals("Nonlinear QSAR: artificial neural network for the Daphnia magna reproduction test",
		table.getValue(1, "title"));
	...
	Assert.assertEquals(PublishedStatus.archived.name(),
		table.getValue(0, ReadProtocol.fields.published_status.name()));
	Assert.assertEquals(PublishedStatus.published.name(),
		table.getValue(1, ReadProtocol.fields.published_status.name()));
	c.close();
    }

    @Test
    public void testCreateEntryFromMultipartWeb() throws Exception {
	String url = createEntryFromMultipartWeb(new Reference(String.format("http://localhost:%d%s", port,
		Resources.protocol)));

	testGet(String.format("%s%s", url, Resources.document), MediaType.APPLICATION_PDF);
	...
	Assert.assertEquals(1, table.getRowCount());
	// Assert.assertEquals(new BigInteger("1"), table.getValue(0,
	// "version"));
	// Assert.assertEquals(new BigInteger("3"), table.getValue(0,
	// "iduser"));
	...
	URL url = getClass().getClassLoader().getResource("net/idea/qmrf/QMRF-NEW.xml");
	...
	for (ReadProtocol.fields field : ReadProtocol.entryFields) {
	    ...
	    case user_uri: {
		values[i] = String.format("http://localhost:%d%s/%s", port, Resources.user, "U88");
		break;
	    }
	    case author_uri: {
		values[i] = String.format("http://localhost:%d%s/%s", port, Resources.user, "U88");
		...
		values[i] = String.format("http://localhost:%d%s/%s", port, Resources.user, "U3");
		break;
	    }
	    case status: {
	...
	MultipartEntity rep = getMultipartWebFormRepresentation(names, values, file,
	...
	RemoteTask task = testAsyncPoll(uri, MediaType.TEXT_URI_LIST, rep, method);
	// wait to complete
	while (!task.isDone()) {
	    task.poll();
	    Thread.sleep(100);
	...
	testGet(String.format("http://localhost:%d%s/%s%s", port, Resources.protocol, id2v1, Resources.document),
