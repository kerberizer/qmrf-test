...
public abstract class QMRFQueryResource<Q extends IQueryRetrieval<T>, T extends Serializable> extends
	QueryResource<Q, T> {
	...
	throw new ResourceException(Status.CLIENT_ERROR_UNSUPPORTED_MEDIA_TYPE,"Not implemented");
    }

    @Override
    protected Task<Reference, Object> addTask(ICallableTask callable, T item, Reference reference)
	    throws ResourceException {
	String name = "task";
	if (item instanceof DBAlert) name = "Saved search";
	if (item instanceof DBProtocol) name = "QMRF update";	
	else
	    name = String.format("%s %s %s", callable.toString(),
			item == null ? "" : item.toString(), reference == null ? "" : " ");
	Task<Reference,Object> task = ((TaskApplication) getApplication()).addTask(name, callable,
	...
	try {
		...
		while (retry < maxRetry) {
			...
			if (r != null)
			    return r;
			batchException = batchException == null ? new RResourceException(Status.SERVER_ERROR_INTERNAL,
				x, variant) : batchException;
